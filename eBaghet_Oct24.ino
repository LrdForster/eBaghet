...
#define CONTROL_RATE 64 //512 // 64 // powers of 2 please
...
int current_note = 0;
int old_note = -1; // Set to -1 so that the first note change will be detected
unsigned long lastChangeTime = 0;

unsigned long sleepThreshold = 25000; // nominal auto off
...
void setup()
{
sleepThreshold = (sleepTime * 1000); // time in milliseconds
...
void deep_sleep() {
 ...
 // Serial.println("Entering deep sleep...");
  ...
  // Disable Serial communication to turn off TX LED
  Serial.end();

  // Disable ADC to save power
  ADCSRA &= ~(1 << ADEN);

  // Disable peripherals that aren't needed
   digitalWrite(LED_BUILTIN, LOW);  // turn the LED OFF (LOW is the voltage level)
   digitalWrite(TX_Pin, HIGH);  // turn the TX LED off (HIGH is the voltage level)
  power_adc_disable();
  power_spi_disable();
  ...
  // Turn off Brown-out Detection (BOD) to save even more power
  sleep_bod_disable();
  ...
  // After this point, the Arduino will stay in deep sleep and will not wake up
  // The system will only wake up with an external reset or power cycle
}


void check() {
    lastChangeTime = millis();
   // Serial.println("Note changed."); // Debugging: Print a message to confirm note change
    if (millis() - lastChangeTime > sleepThreshold) {
    Serial.println("Entering deep sleep..."); // Debugging: Confirm reaching sleep condition
    deep_sleep ();
  }
}
...
void updateAudio()
{
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneGHB.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 9 );
	...
	else if ( instrument == BGT )
	{
		...
		if ( usedrones == DRONE_OFF )
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME ) >> 9 );
			audio_out_2 = 0;
		}
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneBGT.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			if ( startchanter != 0 )
			...
			audio_out_2 = ( int16_t ) ( (  ( int32_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneBRD.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 9 );
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneSML.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 9 );
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneUIL.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 9 );
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneGAL.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 9 );
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 9 );
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( int16_t ) ( ( ( int32_t ) droneAST.next() * DRONES_VOLUME ) >> 9 );
		}
		else
		{
			audio_out_1 = ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 9 );
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneGHB.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 3;
	...
	else if ( instrument == BGT )
	{
		...
		if ( usedrones == DRONE_OFF )
		{
			audio_out_1 = ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME ) >> 3;
			audio_out_2 = 0;
		}
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			...
			audio_out_2 = ( ( int16_t ) droneBGT.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			if ( startchanter != 0 )
			...
			audio_out_2 = ( ( int16_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 3;
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneBRD.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 3;
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneSML.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 3;
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneUIL.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 3;
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneGAL.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 3;
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				audio_out_1 = ( ( int16_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 3;
			}
			else
			{
				audio_out_1 = 0;
			...
			audio_out_2 = ( ( int16_t ) droneAST.next() * DRONES_VOLUME ) >> 3;
		}
		else
		{
			audio_out_1 = ( ( int16_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 3;
...
int updateAudio()
{
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME + ( int32_t ) droneGHB.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneGHB.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	else if ( instrument == BGT )
	{
		...
		if ( usedrones == DRONE_OFF )
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME ) >> 5 );
		}
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int32_t ) droneBGT.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneBGT.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int32_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 6 );
			}
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME + ( int32_t ) droneBRD.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneBRD.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME + ( int32_t ) droneSML.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneSML.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME + ( int32_t ) droneUIL.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneUIL.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME + ( int32_t ) droneGAL.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneGAL.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME + ( int32_t ) droneAST.next() * DRONES_VOLUME ) >> 6 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneAST.next() * DRONES_VOLUME ) >> 6 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 5 );
		}
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentGHB.next() * CHANTER_VOLUME + ( int16_t ) droneGHB.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneGHB.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentGHB.next() * CHANTER_VOLUME ) << 3;
		}
	...
	else if ( instrument == BGT )
	{
		...
		if ( usedrones == DRONE_OFF )
		{
			return ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME ) << 3;
		}
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int16_t ) droneBGT.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneBGT.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int16_t ) dronemajBGT.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) dronemajBGT.next() * DRONES_VOLUME ) << 2;
			}
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentBRD.next() * CHANTER_VOLUME + ( int16_t ) droneBRD.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneBRD.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentBRD.next() * CHANTER_VOLUME ) << 3;
		}
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentSML.next() * CHANTER_VOLUME + ( int16_t ) droneSML.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneSML.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentSML.next() * CHANTER_VOLUME ) << 3;
		}
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentUIL.next() * CHANTER_VOLUME + ( int16_t ) droneUIL.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneUIL.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentUIL.next() * CHANTER_VOLUME ) << 3;
		}
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentGAL.next() * CHANTER_VOLUME + ( int16_t ) droneGAL.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneGAL.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentGAL.next() * CHANTER_VOLUME ) << 3;
		}
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentAST.next() * CHANTER_VOLUME + ( int16_t ) droneAST.next() * DRONES_VOLUME ) << 2;
			}
			else
			{
				return ( ( int16_t ) droneAST.next() * DRONES_VOLUME ) << 2;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentAST.next() * CHANTER_VOLUME ) << 3;
		}
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME + ( int32_t ) droneGHB.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneGHB.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentGHB.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == BGT )
	{
		...
		if ( usedrones == DRONE_OFF )
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME ) >> 9 );
		}
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int32_t ) droneBGT.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneBGT.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int32_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 10 );
			}
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME + ( int32_t ) droneBRD.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneBRD.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentBRD.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME + ( int32_t ) droneSML.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneSML.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentSML.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME + ( int32_t ) droneUIL.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneUIL.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentUIL.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME + ( int32_t ) droneGAL.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneGAL.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentGAL.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME + ( int32_t ) droneAST.next() * DRONES_VOLUME ) >> 10 );
			}
			else
			{
				return ( int16_t ) ( ( ( int32_t ) droneAST.next() * DRONES_VOLUME ) >> 10 );
			}
		}
		else
		{
			return ( int16_t ) ( ( ( int32_t ) instrumentAST.next() * CHANTER_VOLUME ) >> 9 );
		}
	...
	if ( instrument == GHB )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentGHB.next() * CHANTER_VOLUME + ( int16_t ) droneGHB.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneGHB.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
	...
	else if ( instrument == BGT )
	{
		...
		else if ( droneintonation == DRONE_INT_STANDARD )
		{
			if ( startchanter != 0 )
			{
				return  ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int16_t ) droneBGT.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return  ( ( int16_t ) droneBGT.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
			return ( ( int16_t ) instrumentBGT.next() * CHANTER_VOLUME + ( int16_t ) dronemajBGT.next() * DRONES_VOLUME ) >> 4;
		}
	...
	else if ( instrument == BRD )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentBRD.next() * CHANTER_VOLUME + ( int16_t ) droneBRD.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneBRD.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
	...
	else if ( instrument == SML )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentSML.next() * CHANTER_VOLUME + ( int16_t ) droneSML.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneSML.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
	...
	else if ( instrument == UIL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentUIL.next() * CHANTER_VOLUME + ( int16_t ) droneUIL.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneUIL.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
	...
	else if ( instrument == GAL )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentGAL.next() * CHANTER_VOLUME + ( int16_t ) droneGAL.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneGAL.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
	...
	else if ( instrument == AST )
	{
		...
		if ( usedrones != DRONE_OFF )
		{
			if ( startchanter != 0 )
			{
				return ( ( int16_t ) instrumentAST.next() * CHANTER_VOLUME + ( int16_t ) droneAST.next() * DRONES_VOLUME ) >> 4;
			}
			else
			{
				return ( ( int16_t ) droneAST.next() * DRONES_VOLUME ) >> 4;
			}
		}
		else
		{
...
void loop()
{
 ...
 check(); // call sub routine to auto off
}
#if (TOUCHMODE == TOUCH_CAP)
